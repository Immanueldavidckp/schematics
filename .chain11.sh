...
echo "=== chain v11 start $(date)"
export _JAVA_OPTIONS="-Xmx1500m"
for CY in 1 2 3 4; do
  echo "=== V11 CYCLE $CY $(date +%H:%M)"
  ...
  kicad-cli pcb drc --severity-error -o /home/david/pcb/.v11cy$CY.rpt telematics-tracker.kicad_pcb >/dev/null 2>&1
  U=$(grep -cE '^\[unconnected' /home/david/pcb/.v11cy$CY.rpt)
  E=$(grep -E '^\[' /home/david/pcb/.v11cy$CY.rpt | grep -vc unconnected)
  echo "V11 CYCLE $CY END: unconnected=$U errors=$E"
  git add -A >/dev/null 2>&1; git commit -q -m "hybrid v11 cycle $CY: unconnected=$U errors=$E" >/dev/null 2>&1
